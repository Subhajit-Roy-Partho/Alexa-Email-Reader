...
const { setDefaultAccount } = accountsModule;
...
export async function POST(request) {
  try {
    ...
    await setDefaultAccount(session.userId, String(formData.get('accountId') || ''));

    return NextResponse.redirect(new URL('/dashboard?saved=default', request.url));
  } catch (error) {
